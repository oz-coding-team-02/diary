...
class QuestionRepository:
    ...
    async def get_seen_question_ids_for_user(self, user_id: int) -> list[int]:
        seen_question_ids = await UserQuestion.filter(user_id=user_id).values_list('question_id', flat=True)
        return list(seen_question_ids) 

    async def get_random_question_excluding_ids(self, excluded_ids: list[int]) -> Optional[QuestionRead]:
       
        question = await Question.filter(id__not_in=excluded_ids).annotate(idx=Random()).order_by("idx").first()
        if question:
