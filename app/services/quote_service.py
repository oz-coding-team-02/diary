...
class QuoteService:
    ...
    async def get_random_quote_or_none(user_id: int) -> QuoteRead:
        ...
        is_bookmarked = await QuoteRepository.get_bookmark_or_none(user_id=user_id, quote_id=quote.id) is not None
        
        return QuoteRead(content=quote.content, author=quote.author, is_bookmarked=is_bookmarked)

    @staticmethod
    async def toggle_bookmark(data: BookmarkCreate) -> bool:
        bookmark = await QuoteRepository.get_bookmark_or_none(user_id=data.user_id, quote_id=data.quote_id)
        
        if bookmark:
            await bookmark.delete()
            return False
        else:
            await QuoteRepository.create_bookmark(user_id=data.user_id, quote_id=data.quote_id)
            return True
        
    @staticmethod
    async def get_bookmarked_quotes_for_user(user_id: int) -> list[BookmarkedQuoteRead]:
        bookmarks = await QuoteRepository.get_bookmarked_quotes_for_user(user_id=user_id)
        result_list = []
