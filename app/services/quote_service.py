...
class QuoteService:
    ...
    async def get_random_quote_or_none(self, user_id: int) -> QuoteRead:
        ...
        is_bookmarked = await self.repo.get_bookmark_or_none(user_id=user_id, quote_id=quote.id) is not None
    ...
    async def toggle_bookmark(self, data: BookmarkCreate) -> bool:
        bookmark = await self.repo.get_bookmark_or_none(user_id=data.user_id, quote_id=data.quote_id)
        
        if bookmark:
            await bookmark.delete()
            return False
        else:
            await self.repo.create_bookmark(user_id=data.user_id, quote_id=data.quote_id)
            return True
        
    async def get_bookmarked_quotes_for_user(self, user_id: int) -> list[BookmarkedQuoteRead]:
